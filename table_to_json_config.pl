...
while ($s = <FLDFILE>) {
    if ($s =~ m/^;/) { #;comment - do nothing
      ...
      # resolve #MESS(..) references
      if ( $value =~ m/#MESS\(.+?\)/i ) {
        $value =~ s/(#MESS\(.+?\))/::MsgTrn($1)/ieg;
        ::Prt("-T", "  resolved to [$keyword=$value]\n" ) if $_Debug;
      }

      $sectkey = $section . "_" . $keyword;
      $sectkey =~ s/\s//g;
      if (!$allowdupes) {
        if (defined($$iniref{$sectkey})) {
          if ($sectkey !~ m/\$/) { #ignore keywords with dollars in them, they are crap from Perl in HYSCRIPT jobs
            Prt("-W","Duplicate keyword $sectkey in INI file $openname\n");
          }
        }
      }
