...
sub create_config_json{
  ...
  foreach my $header_count ( 0..$#headers){
    ...
    $data->{'elements'}[$header_count]{'foreign_field'} = lc($headers[$header_count]);
    $data->{'elements'}[$header_count]{'foreign_table'} = lc($table_name);
    $data->{'elements'}[$header_count]{'foreign_key_field'} = '';
